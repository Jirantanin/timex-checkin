...
(async () => {
  ...
  // Try manual-checkin with Reason
  console.log('=== Manual check-in with Reason ===');
  ...
  // Try manual check-out too
  console.log('\n=== Manual check-out with Reason ===');
  const manualCoRes = await page.request.post(API_BASE + '/Attendance/manual-checkin', {
    data: {
      UserId: EMPLOYEE_ID,
      Latitude: 16.2468,
      Longitude: 103.2510,
  ...
  console.log('manual-checkout:', manualCoRes.status(), await manualCoRes.text());
